...
export default function Page() {
  ...
  const { data, isLoading } = useSimpleMaster<DesignationRecord>("designation");
  ...
  const handleToggle = async (item: DesignationRecord) => {
    try {
      const id = String(item.DesignationId ?? item.Id ?? "");
      await entityCall("designation", "update", {
        ...item,
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await entityCall("designation", "delete", { Id: deleteId });
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <section className="rounded-2xl border border-gray-200 bg-white shadow-sm transition-all dark:border-slate-800 dark:bg-slate-900/50 dark:backdrop-blur-xl">
        ...
        <UniversalTable
          columns={masterTableColumns.designation}
          idKey={masterIdKeys.designation}
          data={filtered}
          loading={isLoading}
          onEdit={(item) => {
