    ...
    <div className="overflow-hidden">
          ...
          <tbody className="divide-y divide-gray-100 bg-white">
            ...
            {!loading &&
              data.map((dealer) => {
                ...
                return (
                  <tr key={dealer.Id} className="transition hover:bg-cyan-50/30">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="font-semibold text-gray-950">{dealer.DealerName || "Untitled dealer"}</div>
                      </div>
                    </td>
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-start gap-2 text-sm text-gray-700">
                        ...
                        <div>
                          <div>{getCityName(dealer.CityId)}</div>
                          <div className="text-xs text-gray-500 max-w-[180px] truncate">{dealer.Address}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-4">
              ...
              <article key={dealer.Id} className="p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <h3 className="truncate font-semibold text-gray-950">{dealer.DealerName || "Untitled dealer"}</h3>
                      <p className="truncate text-sm text-gray-500">{dealer.Email || "No email"}</p>
                    </div>
                  </div>
                  <button
                    onClick={() => onToggleActive(dealer)}
                ...
                <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                  <div className="rounded-lg bg-gray-50 p-3">
                    ...
                    <div className="mt-1 truncate text-gray-800">{dealer.Mobile || "—"}</div>
                  </div>
                  <div className="rounded-lg bg-gray-50 p-3">
                    <div className="text-xs font-medium text-gray-500">City</div>
                    <div className="mt-1 truncate text-gray-800">{getCityName(dealer.CityId)}</div>
                  </div>
                </div>
      ...
      {!loading && data.length === 0 && (
        <div className="flex flex-col items-center justify-center px-6 py-16 text-center">
          ...
          <h3 className="mt-4 text-base font-semibold text-gray-950">No dealers found</h3>
