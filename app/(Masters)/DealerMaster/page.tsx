...
import { useDealerMaster } from "@/hooks/useMasters";
import { isMasterActive } from "@/lib/utils/masterStatus";
import type { DealerRecord } from "@/types/master";
...
export default function Page() {
  ...
  const { data, cities, isLoading, isLookupLoading } = useDealerMaster();
  ...
  const handleToggle = async (item: DealerRecord) => {
    try {
      const id = String(item.DealerId ?? item.Id ?? "");
      await entityCall("dealer", "update", {
        ...item,
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await entityCall("dealer", "delete", { Id: deleteId });
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <section className="rounded-2xl border border-gray-200 bg-white shadow-sm transition-all dark:border-slate-800 dark:bg-slate-900/50 dark:backdrop-blur-xl">
        <div className="flex flex-col gap-4 border-b border-gray-100 p-6 dark:border-slate-800 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h2 className="text-xl font-bold text-gray-950 dark:text-white">Active Dealers</h2>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-500">
              Showing {filtered.length} of {dealers.length} records shown
            </p>
          </div>
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
            ...
            >
              ...
              <option value="active">Active Dealers</option>
              <option value="inactive">Off-boarded Only</option>
            </select>
          </div>
        </div>

        <UniversalTable
          columns={masterTableColumns.dealer}
          idKey={masterIdKeys.dealer}
          data={filtered}
          loading={isLoading || isLookupLoading}
          onEdit={(dealer) => {
            router.push(`/DealerMaster/edit/${dealer.DealerId || dealer.Id}`);
          }}
