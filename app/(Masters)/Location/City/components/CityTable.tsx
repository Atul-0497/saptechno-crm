    ...
    <div className="overflow-hidden">
          ...
          <tbody className="divide-y divide-gray-100 bg-white dark:divide-slate-800 dark:bg-transparent">
            ...
            {!loading &&
              data.map((city) => {
                ...
                return (
                  <tr key={city.Id} className="transition hover:bg-cyan-50/30 dark:hover:bg-slate-800/40">
                    <td className="px-5 py-4 text-left">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="font-semibold text-gray-950 dark:text-white text-left">{city.CityName || "Untitled city"}</div>
                      </div>
                    </td>
                    <td className="px-5 py-4 text-left">
                      <div className="text-sm font-bold text-gray-700 dark:text-slate-300">{getStateName(city.StateId)}</div>
                    </td>
                    <td className="px-5 py-4 text-left">
                      <button
      ...
      <div className="divide-y divide-gray-100 lg:hidden text-left dark:divide-slate-800">
        ...
        {!loading &&
          data.map((city) => {
            ...
            return (
              <article key={city.Id} className="p-4 text-left">
                <div className="flex items-start justify-between gap-3 text-left">
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <h3 className="truncate font-bold text-gray-950 dark:text-white">{city.CityName || "Untitled city"}</h3>
                      <p className="truncate text-xs font-bold text-gray-500 dark:text-slate-400">{getStateName(city.StateId)}</p>
                    </div>
                  </div>
                  <button
                    onClick={() => onToggleActive(city)}
      ...
      {!loading && data.length === 0 && (
        <div className="flex flex-col items-center justify-center px-6 py-16 text-center">
          ...
          <h3 className="mt-4 text-base font-bold text-gray-950 dark:text-white text-left">No cities found</h3>
