...
import DeleteConfirmModal from "../City/components/DeleteConfirmModal";
...
export default function Page() {
  ...
  const { data: pincodes, isLoading, update, remove } = usePincodeMaster();
  const [deleteId, setDeleteId] = useState<string | null>(null);
  const [query, setQuery] = useState("");
  const [status, setStatus] = useState<"all" | "active" | "inactive">("all");
  const activeCount = pincodes.filter((p) => isMasterActive(p.Active)).length;
  ...
  }, [pincodes, cities, states, query, status]);
  ...
  const handleToggle = async (p: PincodeRecord) => {
    try {
      await update.mutateAsync({
        ...p,
        PincodeId: String(p.PincodeId ?? p.Id),
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div className="max-w-2xl">
              ...
              <p className="mt-3 text-base leading-relaxed text-gray-500 dark:text-gray-400">
                Maintain a master list of postal codes linked to specific cities and states for automated data entry.
              </p>
            </div>

            <Link
              href="/Location/Pincode/add"
      ...
      <section className="rounded-2xl border border-gray-200 bg-white shadow-sm transition-all dark:border-slate-800 dark:bg-slate-900/50 dark:backdrop-blur-xl">
        <div className="flex flex-col gap-4 border-b border-gray-100 p-6 dark:border-slate-800 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h2 className="text-xl font-bold text-gray-950 dark:text-white">Pin Code Registry</h2>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-500">
              Showing {filtered.length} of {pincodes.length} available codes
            </p>
          </div>
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
            <label className="relative block w-full sm:w-80">
              ...
              <input
                ...
                placeholder="Search code, city or state..."
        ...
        <PincodeTable
          ...
          loading={isLoading}
          onEdit={(p) => {
            router.push(`/Location/Pincode/edit/${p.PincodeId || p.Id}`);
          }}
