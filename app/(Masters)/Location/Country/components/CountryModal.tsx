...
type CountryModalProps = {
  ...
  onClose: () => void;
  onSubmit: (form: CountryFormValues) => void | Promise<void>;
  submitting: boolean;
};
...
export default function CountryModal({
  ...
  useEffect(() => {
    if (data) {
      ...
      } as CountryFormValues);
    } else {
      setForm({ CountryName: "", CountryCode: "", Active: "1" });
    }
  }, [data]);

  const updateField =
    (field: keyof CountryFormValues) =>
      ...
      >
        ...
        <div className="space-y-5 px-6 py-6 text-left">
          <label className="block">
            <span className="text-sm font-bold text-gray-700 dark:text-slate-300">Country Name</span>
            <input
              value={form.CountryName || ""}
              onChange={updateField("CountryName")}
          ...
          <label className="block">
            <span className="text-sm font-bold text-gray-700 dark:text-slate-300">Code (ISO)</span>
            <input
          ...
          <div className="flex items-center justify-between gap-4 rounded-xl border border-gray-200 px-4 py-4 dark:border-slate-800 dark:bg-slate-950/30 text-left">
            <span>
              <span className="block text-sm font-bold text-gray-700 dark:text-slate-300">Active Status</span>
              <span className="text-xs text-gray-500 dark:text-slate-500">Visible in location selectors</span>
            </span>
            <select
