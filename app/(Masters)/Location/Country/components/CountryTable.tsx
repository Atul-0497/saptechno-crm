    ...
    <div className="overflow-hidden">
          ...
          <tbody className="divide-y divide-gray-100 bg-white dark:divide-slate-800 dark:bg-transparent">
            ...
            {!loading &&
              data.map((country) => {
                ...
                return (
                  <tr key={country.Id} className="transition hover:bg-cyan-50/30 dark:hover:bg-slate-800/40">
                    <td className="px-5 py-4 text-left">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="font-semibold text-gray-950 dark:text-white text-left">{country.CountryName || "Untitled country"}</div>
                      </div>
                    </td>
                    <td className="px-5 py-4 text-left">
                      <span className="font-mono text-xs font-bold text-gray-600 bg-gray-100 px-2 py-1 rounded dark:bg-slate-800 dark:text-slate-300 dark:border dark:border-slate-700">
                        {country.CountryCode || "—"}
                      </span>
                    </td>
                    <td className="px-5 py-4 text-left">
                ...
                <div className="flex items-start justify-between gap-3 text-left">
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <h3 className="truncate font-bold text-gray-950 dark:text-white">{country.CountryName || "Untitled country"}</h3>
                      <p className="text-xs font-bold text-gray-500 dark:text-slate-400 font-mono">{country.CountryCode || "Code N/A"}</p>
                    </div>
                  </div>
                  <button
                    onClick={() => onToggleActive(country)}
      ...
      {!loading && data.length === 0 && (
        <div className="flex flex-col items-center justify-center px-6 py-16 text-center">
          ...
          <h3 className="mt-4 text-base font-bold text-gray-950 dark:text-white">No countries found</h3>
          <p className="mt-2 max-w-sm text-sm font-bold text-gray-500 dark:text-slate-400">
