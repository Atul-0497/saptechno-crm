...
import { useStateMaster } from "@/hooks/useMasters";
import { isMasterActive } from "@/lib/utils/masterStatus";
import type { StateRecord } from "@/types/master";
...
export default function Page() {
  ...
  const { countries, data, isLoading } = useStateMaster();
  ...
  const handleToggle = async (state: StateRecord) => {
    try {
      const id = String(state.StateId ?? state.Id ?? "");
      await entityCall("state", "update", {
        ...state,
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await entityCall("state", "delete", { Id: deleteId });
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <section className="rounded-2xl border border-gray-200 bg-white shadow-sm transition-all dark:border-slate-800 dark:bg-slate-900/50 dark:backdrop-blur-xl">
        <div className="flex flex-col gap-4 border-b border-gray-100 p-6 dark:border-slate-800 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              Showing {filtered.length} of {states.length} states shown
            </p>
          </div>
        ...
        <UniversalTable
          columns={masterTableColumns.state}
          idKey={masterIdKeys.state}
          data={filtered}
          loading={isLoading}
          onEdit={(s) => {
