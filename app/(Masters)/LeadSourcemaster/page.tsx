...
export default function Page() {
  ...
  const { data, isLoading } = useLeadSourceMaster();
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <section className="rounded-2xl border border-gray-200 bg-white shadow-sm transition-all dark:border-slate-800 dark:bg-slate-900/50 dark:backdrop-blur-xl">
        <div className="flex flex-col gap-4 border-b border-gray-100 p-6 dark:border-slate-800 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h2 className="text-xl font-bold text-gray-950 dark:text-white">Active Channels</h2>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-500">
              Showing {filtered.length} of {sources.length} records shown
            </p>
          </div>
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
            ...
            >
              ...
              <option value="inactive">Deprecated Only</option>
            </select>
          </div>
        </div>

        <UniversalTable
          columns={masterTableColumns.leadsource}
          idKey={masterIdKeys.leadsource}
          data={filtered}
          loading={isLoading}
          onEdit={(item) => {
