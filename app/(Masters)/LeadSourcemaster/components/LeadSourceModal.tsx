...
type LeadSourceModalProps = {
  open: boolean;
  data: LeadSourceRecord | null;
  onClose: () => void;
  onSubmit: (form: LeadSourceFormValues) => void | Promise<void>;
  submitting: boolean;
};
...
export default function LeadSourceModal({
  ...
  const [form, setForm] = useState<LeadSourceFormValues>({
    Name: "",
    Active: "1",
  });
  ...
  const updateField =
    (field: keyof LeadSourceFormValues) =>
      ...
      >
        ...
        <div className="space-y-5 px-6 py-6">
          <label className="block">
            <span className="text-sm font-bold text-gray-700 dark:text-slate-300">Source Name</span>
            <input
              value={form.Name || ""}
          ...
          <div className="flex items-center justify-between gap-4 rounded-xl border border-gray-200 px-4 py-4 dark:border-slate-800 dark:bg-slate-950/30">
            <span>
              ...
              <span className="text-xs text-gray-500 dark:text-slate-500">Enable for lead forms</span>
            </span>
            <select
